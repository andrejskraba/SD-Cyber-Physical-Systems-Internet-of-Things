...
io.sockets.on("connection", function(socket) {
    ...
    socket.on("ukazArduinu", function(stevilkaUkaza){
        if (stevilkaUkaza == "1") {
            socket.emit("sporociloKlientu", "LED 1 prižgana.");
            board.digitalWrite(13, board.HIGH); // na pinu 13 zapišemo 1
        }
        if (stevilkaUkaza == 0) {
            socket.emit("sporociloKlientu", "LED 1 ugasnjena.");
            board.digitalWrite(13, board.LOW); // na pinu 13 zapišemo 0
        }
        if (stevilkaUkaza == 2) {
            socket.emit("sporociloKlientu", "LED 2 ugasnjena.");
            board.digitalWrite(8, board.LOW); // na pinu 12 zapišemo 0
        }
        if (stevilkaUkaza == 3) {
            socket.emit("sporociloKlientu", "LED 2 prižgana.");
            board.digitalWrite(8, board.HIGH); // na pinu 12 zapišemo 1
        }
    });
});